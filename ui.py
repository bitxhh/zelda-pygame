...
class UI:
    ...
    def show_exp(self, exp, paused):
        text_surf = self.font.render(str(int(exp)), False, TEXT_COLOR)
        text_rect = text_surf.get_rect(bottomright=(self.exp_x - 20, self.exp_y - 20)) if not paused \
            else text_surf.get_rect(midbottom=(self.exp_x // 2, self.exp_y - 20))
    ...
    def update_bars(self, player):
        self.health_bar_rect = pygame.Rect(10, 10, HEALTH_BAR_WIDTH * (player.stats['health']/player.start_health),
                                           BAR_HEIGHT)
        self.energy_bar_rect = pygame.Rect(10, 34, ENERGY_BAR_WIDTH * (player.stats['energy']/player.start_energy),
                                           BAR_HEIGHT)

    def displey(self, player, paused):
        self.show_exp(player.exp, paused)
        self.update_bars(player)
